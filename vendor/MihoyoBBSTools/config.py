...
runtime_config_path = f"{path}/{config_prefix}runtime.yaml"


def runtime_cookie_enabled() -> bool:
    value = os.getenv("MIHOYO_USE_RUNTIME_COOKIE")
    if value is None:
        return True
...
def load_config(p_path=None):
    ...
    if runtime_cookie_enabled() and os.path.exists(runtime_config_path):
        with open(runtime_config_path, "r", encoding='utf-8') as runtime_file:
            runtime_data = yaml.load(runtime_file, Loader=yaml.FullLoader) or {}
        if runtime_data:
            data = deep_merge(data, runtime_data)
    # 去除cookie最末尾的空格
